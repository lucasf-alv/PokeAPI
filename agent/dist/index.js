...
import { z } from "zod";
import { Agent, MCPServerStdio, OpenAIChatCompletionsModel, run, setTracingDisabled, } from "@openai/agents";
const app = express();
const PORT = 3000;
app.use(cors());
...
async function getPokemonFromMcp(name) {
...
app.post("/chat", async (req, res) => {
    try {
        ...
        const pokemon = await Promise.all(output.pokemonNames.map((name) => getPokemonFromMcp(name)));
        return res.json({
            view: output.view,
            answer: output.answer,
            pokemon,
