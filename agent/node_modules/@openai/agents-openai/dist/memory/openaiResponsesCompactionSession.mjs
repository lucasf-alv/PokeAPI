...
export class OpenAIResponsesCompactionSession {
    ...
    sessionItems;
    mutationOperation = Promise.resolve();
    mutationGeneration = {};
    compactionOwnership = new WeakMap();
    constructor(options) {
    ...
    async runCompaction(args = {}, _runContext, ownership) {
        return this.runMutationOperation(async () => {
            if (ownership !== undefined && !this.ownsCompaction(ownership)) {
                logger.debug('skip: automatic compaction no longer owns session history');
                return null;
            }
    ...
    async runCompactionOperation(args) {
        ...
        const outputCompactionCandidateItems = selectCompactionCandidateItems(outputItems);
    ...
    async getItemsWithCompactionOwnership(_runContext) {
        return this.runMutationOperation(async () => {
    ...
    prepareHistoryItemsForPersistenceComparison(items) {
        const prepare = this.underlyingSession.prepareHistoryItemsForPersistenceComparison;
        return prepare ? prepare.call(this.underlyingSession, items) : items;
    }
    async addItems(items) {
        await this.addItemsWithCompactionOwnership(items, null);
    }
    async addItemsWithCompactionOwnership(items, ownership, _runContext) {
        if (items.length === 0) {
            return;
        }
