...
function appendAudioDelta(snapshot, rawAudio) {
    if (!isRecord(rawAudio)) {
        throw new agents_core_1.ModelBehaviorError('Chat Completions stream returned malformed audio output: expected delta.audio to be an object.');
    }
    ...
    for (const [key, value] of Object.entries(rawAudio)) {
        ...
        if (key === 'data') {
            if (typeof value !== 'string') {
                throw new agents_core_1.ModelBehaviorError('Chat Completions stream returned malformed audio output: expected delta.audio.data to be a string.');
            }
            audio.data = `${audio.data ?? ''}${value}`;
        }
        else if (key === 'transcript') {
            if (typeof value !== 'string') {
                throw new agents_core_1.ModelBehaviorError('Chat Completions stream returned malformed audio output: expected delta.audio.transcript to be a string.');
            }
...
async function* convertChatCompletionsStreamToResponses(response, stream, options = {}) {
    ...
    for await (const chunk of stream) {
        ...
        if (chunk.usage) {
            ...
            if (options.preserveRawUsage === true) {
                rawUsage = (0, internal_1.snapshotRawUsage)(usagePayload);
            }
            usage = rawUsage ?? usagePayload;
        }
        if (chunk.id && (response.id === openaiItemIds_1.FAKE_ID || !response.id)) {
        ...
        if (audioDelta &&
            ...
            if (rawAudio !== undefined && rawAudio !== null) {
                ...
                try {
                    ...
                    throw new agents_core_1.ModelBehaviorError('Chat Completions stream returned malformed audio output: expected delta.audio to be cloneable.');
                }
        ...
        if (delta.content) {
            if (!state.text_content) {
                ...
                state.messageItemId =
                    response.id && response.id !== openaiItemIds_1.FAKE_ID ? response.id : undefined;
            }
            yield {
            ...
            for (const tc_delta of delta.tool_calls) {
                ...
                if (!(tc_delta.index in state.function_calls)) {
                    state.function_calls[tc_delta.index] = {
                        id: response.id || openaiItemIds_1.FAKE_ID,
                        arguments: '',
                        name: '',
                        type: 'function_call',
                        callId: '',
    ...
    const outputs = [];
    const outputItemId = response.id || openaiItemIds_1.FAKE_ID;
    ...
    if (state.audio) {
        ...
        if (typeof data !== 'string') {
            throw new agents_core_1.ModelBehaviorError('Chat Completions stream ended with audio output but no audio data.');
        }
        audioContent = {
            type: 'audio',
            audio: data,
    ...
    if ((0, openaiChatCompletionsTruncation_1.isTruncatedEmptyChatCompletion)({
        finishReason: state.finishReason,
        ...
        throw (0, openaiChatCompletionsTruncation_1.createTruncatedEmptyChatCompletionError)();
    }
