...
import { createTruncatedEmptyChatCompletionError, isTruncatedEmptyChatCompletion, isTruncatedEmptyChatCompletionError, } from "./openaiChatCompletionsTruncation.mjs";
export { FAKE_ID };
...
export class OpenAIChatCompletionsModel {
    ...
    async getResponse(request) {
        ...
        const { response, rawResponse, rawUsage, preservedUsage } = await withGenerationSpan(async (span) => {
            ...
            const preservedUsage = rawUsage !== undefined
                ? new Usage(toResponseUsage(rawUsage))
                : undefined;
    ...
    async *getStreamedResponse(request) {
        ...
        try {
            ...
            const truncatedResponse = response !== undefined && isTruncatedEmptyChatCompletionError(error)
                ? response
                : undefined;
            if (truncatedResponse) {
                reportModelFailureUsage(request, error, new Usage(toResponseUsage(truncatedResponse.usage ?? {
                    prompt_tokens: 0,
                    completion_tokens: 0,
                    total_tokens: 0,
                })));
