...
class OpenAIRealtimeBase extends utils_3.EventEmitterDelegate {
    ...
    resetHistory(oldHistory, newHistory) {
        ...
        const isMcpItem = (item) => item.type === 'mcp_call' ||
            item.type === 'mcp_tool_call' ||
            item.type === 'mcp_approval_request';
        const oldMcpItemIds = new Set(oldHistory.filter(isMcpItem).map((item) => item.itemId));
        const rejectedMcpItem = [...additions, ...updates].find((item) => isMcpItem(item) || oldMcpItemIds.has(item.itemId));
        if (rejectedMcpItem) {
            const replaced = oldHistory.find((item) => item.itemId === rejectedMcpItem.itemId);
            const detail = replaced && isMcpItem(replaced)
            ...
            throw new agents_core_1.UserError(`MCP items cannot be added or updated via updateHistory(). Found ${detail}. Leave existing MCP items unchanged, or remove them from history entirely, before calling updateHistory().`);
        }
