...
exports.getSerializedOutputGuardrailResults = getSerializedOutputGuardrailResults;
...
exports.finalizeOutputGuardrails = finalizeOutputGuardrails;
exports.sanitizeBlockedOutputGuardrailError = sanitizeBlockedOutputGuardrailError;
exports.sanitizeBlockedOutputGuardrailErrorDetails = sanitizeBlockedOutputGuardrailErrorDetails;
exports.sanitizeBlockedOutputGuardrailResults = sanitizeBlockedOutputGuardrailResults;
exports.replaceSanitizedOutputGuardrailMessages = replaceSanitizedOutputGuardrailMessages;
exports.sanitizeBlockedToolOutputGuardrailResults = sanitizeBlockedToolOutputGuardrailResults;
...
function getSerializedOutputGuardrailResults(state, blockedOutput) {
    const results = new Set();
    for (const result of state._outputGuardrailResults) {
        if (result.agent !== state._currentAgent) {
            continue;
        }
        if (result.agentOutput !== blockedOutput ||
            result.output.outputInfo !== undefined) {
            throw new errors_1.UserError('Cannot resume this serialized terminal output because previous output guardrail result ownership was not preserved. Continue the live RunState or start a new run from safe input.');
        }
        results.add(result);
    }
    return results;
...
function getCurrentResponseOutputGuardrailResults(state, guardedTerminalToolOutput, redactedOutput) {
    ...
    const results = guardedTerminalToolOutput &&
...
async function runGuardrailsWithTripwire(options) {
    ...
    const guardrailPromises = guardrails.map(async (guardrail) => {
        const observedResult = await (0, tracing_1.withGuardrailSpan)(async (span) => {
...
async function runInputGuardrails(state, guardrails, options = {}) {
    ...
    return await runGuardrailsWithTripwire({
        ...
        onError: (error) => {
            return new errors_1.GuardrailExecutionError(`Input guardrail failed to complete: ${error}`, error, state);
        },
        onErrorObserved: options.onErrorObserved,
    });
}
async function runOutputGuardrails(state, runnerOutputGuardrails, output, onResultObserved) {
    ...
    const agentOutput = state._currentAgent.outputType === 'text'
        ...
        : (0, finalOutputError_1.processFinalOutputWithRedaction)(() => state._currentAgent.processFinalOutput(output));
    const runOutput = (0, items_1.getRunOutput)(state._generatedItems, state._reasoningItemIdPolicy);
    const guardrailArgs = {
        agent: state._currentAgent,
        agentOutput,
        context: state._context,
    ...
    await runGuardrailsWithTripwire({
        ...
        onError: (error) => {
            return new errors_1.GuardrailExecutionError(`Output guardrail failed to complete: ${error}`, error, state);
        },
        onResultObserved,
    });
}
async function finalizeOutputGuardrails(options) {
    const { state, runnerOutputGuardrails, output, redactedOutput, guardedTerminalToolOutput, signal, sanitizeRejectedOutput, persistBlockedOutput, persistUnblockedFailure, onBlockedOutput, releaseBlockedOutputPersistence, } = options;
    ...
    try {
        ...
        const currentTripwireResult = state._outputGuardrailResults
            .slice(outputGuardrailResultStart)
            .find((result) => ownedGuardrailResults.has(result) &&
            observedResults.get(result) === true) ?? completedTripwireResult;
        const outputBlockedResult = sanitizeRejectedOutput(state, outputGuardrailResultStart, currentTripwireResult, observedResults, error instanceof errors_1.OutputGuardrailTripwireTriggered ? error : undefined, ownedGuardrailResults);
        const outputBlockedMessage = typeof outputBlockedResult === 'string' || outputBlockedResult === false
            ? outputBlockedResult
        ...
        const guardrailError = outputBlocked
            ? sanitizeBlockedOutputGuardrailError(error, output, blockedMessage, state, currentTripwireResult)
            : error;
        const completedOutputTripwireError = completedTripwireResult !== undefined &&
            guardrailError instanceof errors_1.OutputGuardrailTripwireTriggered;
        if (outputBlocked || completedOutputTripwireError) {
            ...
            if (persistBlockedOutput && !signal?.aborted) {
                try {
                    ...
                    if (guardrailError instanceof errors_1.AgentsError) {
                        guardrailError.state ??= state;
                    }
                    if (guardrailError instanceof errors_1.OutputGuardrailTripwireTriggered ||
                        !(0, finalOutputError_1.isDataRedactedError)(guardrailError)) {
                        guardrailError.cause =
                            outputBlocked
                                ? sanitizeBlockedOutputGuardrailErrorDetails(persistenceError, output, blockedMessage, state)
                                : persistenceError;
                    }
                }
            }
            ...
            try {
                ...
                if (!(0, finalOutputError_1.isDataRedactedError)(guardrailError)) {
                    guardrailError.cause =
                        persistenceError;
                }
            }
...
function sanitizeBlockedOutputGuardrailError(error, rejectedOutput, redactedOutput, state, currentTripwireResult) {
    if (error instanceof errors_1.OutputGuardrailTripwireTriggered) {
        ...
        return new errors_1.OutputGuardrailTripwireTriggered('Output guardrail triggered.', completedResult, state);
    }
    if (!(error instanceof errors_1.GuardrailExecutionError)) {
        return error;
    }
    return new errors_1.GuardrailExecutionError(sanitizeBlockedOutputGuardrailErrorDetails(error, rejectedOutput, redactedOutput, state).message, sanitizeBlockedOutputGuardrailErrorDetails(error.error, rejectedOutput, redactedOutput, state), state);
}
function sanitizeBlockedOutputGuardrailErrorDetails(error, rejectedOutput, redactedOutput, state) {
...
function sanitizeBlockedOutputGuardrailResults(state, resultStartIndex, redactedOutput, observedTripwireResults, tripwire, ownedGuardrailResults = new Set()) {
    const results = state._outputGuardrailResults;
    const sanitizedResults = [];
    const replacements = new Map();
    const sanitizedOwnedResults = new Set();
...
function replaceSanitizedOutputGuardrailMessages(state, sanitizedOwnedResults, redactedOutput, tripwire) {
    const replacements = new Map();
    state._outputGuardrailResults = state._outputGuardrailResults.map((result) => {
        if (!sanitizedOwnedResults.has(result)) {
            return result;
        }
        const replacement = cloneSanitizedOutputGuardrailResult(result, redactedOutput, result.output.tripwireTriggered);
        if (!replacement) {
            return result;
        }
        replacements.set(result, replacement);
...
function sanitizeBlockedToolOutputGuardrailResults(state, resultStartIndex, redactedOutput) {
