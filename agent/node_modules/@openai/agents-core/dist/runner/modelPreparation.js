...
exports.trackDeferredToolRecovery = trackDeferredToolRecovery;
exports.prepareAgentArtifacts = prepareAgentArtifacts;
exports.resolveModelVisibleToolNameCollisions = resolveModelVisibleToolNameCollisions;
...
function trackDeferredToolRecovery(state, sourceAgent, response) {
    if (response.functionToolsNotFound?.some((call) => call.reason === 'not_loaded')) {
 ...
 */
async function prepareAgentArtifacts(state, executionAgent = state._currentAgent, toolNameCollisionPolicy = 'warn') {
    ...
    if (deferredToolRecoveryAgents.get(state) === state._currentAgent &&
        !capabilities.tools.some(toolSearch_1.isDeferredHostedMcpTool)) {
        ...
        capabilities.tools = capabilities.tools.map((tool) => tool.type === 'hosted_tool' &&
...
/** @internal */
function resolveModelVisibleToolNameCollisions(tools, handoffs, collisionPolicy) {
    ...
    for (const [index, tool] of tools.entries()) {
        if (tool.type === 'function') {
            const namespace = (0, toolIdentity_1.getFunctionToolNamespace)(tool);
            ...
            const dispatchName = (0, toolIdentity_1.getFunctionToolQualifiedName)(tool) ?? tool.name;
...
function throwToolNameCollisionError(duplicates) {
    if (logger_1.default.dontLogToolData) {
        throw new errors_1.UserError(`Duplicate enabled function tool or handoff names found. ${toolNameCollisionRemediation}`);
    }
    const label = duplicates.length === 1 ? 'name' : 'names';
    const details = duplicates
        .map(([name, entries]) => `'${name}' (${formatToolKinds(entries.map((entry) => entry.kind))})`)
        .join(', ');
    throw new errors_1.UserError(`Duplicate enabled tool ${label} found: ${details}. ${toolNameCollisionRemediation}`);
}
function warnToolNameCollision(name, entries) {
    if (logger_1.default.dontLogToolData) {
        logger_1.default.warn('Tool name collision detected. Assign unique routed tool names or enable tool data logging for details. Only the current dispatch winner will be exposed.');
        return;
    }
    logger_1.default.warn(`Duplicate enabled tool name found: '${name}' (${formatToolKinds(entries.map((entry) => entry.kind))}). ${toolNameCollisionRemediation} Only the current dispatch winner will be exposed.`);
}
