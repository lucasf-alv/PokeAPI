...
async function resumeInterruptedTurn(options) {
    const { state, runner, toolErrorFormatter, agentToolParentRunConfig, signal, onStepItems, validateHandoffAgent, beforeResumedSideEffects, onHandoff, } = options;
    ...
    const preparation = approvedToolWillResume ||
        (state._lastProcessedResponse?.handoffs.length ?? 0) > 0
        ? await beforeResumedSideEffects?.()
        : undefined;
    let unfilteredHandoffInput;
    const turnResult = await (0, turnResolution_1.resolveInterruptedTurn)(state._currentAgent, state._originalInput, state._generatedItems, state._lastTurnResponse, state._lastProcessedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, validateHandoffAgent, preparation
        ? (input) => {
            unfilteredHandoffInput = state._captureHandoffInput(input.inputHistory, [...input.preHandoffItems, ...input.newItems]);
        }
        : undefined);
    const resumedSideEffects = turnResult.nextStep.type === 'next_step_handoff' ||
    ...
    if (preparation && unfilteredHandoffInput) {
        preparation.handoffInput = state._captureHandoffInput(turnResult.originalInput, turnResult.generatedItems);
        const canonicalInput = state._deserializeHandoffInput(unfilteredHandoffInput);
    ...
    if (turnResult.nextStep.type === 'next_step_handoff') {
        ...
        state.setCurrentAgent(turnResult.nextStep.newAgent);
