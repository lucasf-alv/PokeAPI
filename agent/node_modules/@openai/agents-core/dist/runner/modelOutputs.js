...
function recordMissingFunctionTool(output, toolName, agent, items, toolsUsed, functionToolsNotFound, reason) {
...
function checkDeferredFunctionToolLoaded(toolCall, tool, loadedToolNames, agent, behavior) {
...
function processModelResponse(modelResponse, agent, tools, handoffs, priorItems = [], toolNotFoundBehavior = 'raise_error', processingOptions = {}) {
    ...
    for (const output of modelResponse.output) {
        ...
        if (resolved.type === 'not_found') {
            ...
            if (!checkDeferredFunctionToolLoaded(output, resolved.tool, loadedDeferredToolState.loadedToolNames, agent, toolNotFoundBehavior)) {
                recordMissingFunctionTool(normalizeFunctionToolCallForStorage(output, resolved.tool), (0, toolIdentity_1.getFunctionToolQualifiedName)(resolved.tool) ?? resolved.tool.name, agent, items, toolsUsed, functionToolsNotFound, 'not_loaded');
                continue;
            }
