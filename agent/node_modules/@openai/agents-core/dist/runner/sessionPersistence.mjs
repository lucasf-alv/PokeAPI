...
import { isOpenAIResponsesCompactionAwareSession, isOpenAIResponsesCompactionOwnershipAwareSession, isRunContextAwareSession, isSessionHistoryTransactionAwareSession, } from "../memory/session.mjs";
import { assertPendingSessionWriteOwnership, capturePendingSessionWriteTerminalFinalization, clearPendingSessionWriteTerminalProducer, getPendingSessionWriteAppendItems, } from "../runState.mjs";
...
import { buildRunItemPersistencePlan as buildCanonicalRunItemPersistencePlan, getBlockedOutputSessionSnapshotRunItems, hasBlockedOutputExecutionEffect, hasPendingApprovedToolInputCompaction, hasTerminalToolOutputSource, selectRunItemIndexesForBlockedOutput, } from "./blockedOutputPersistence.mjs";
import { agentItemRangeMatches, normalizeItemsForSessionPersistence, sessionItemArraysMatch, } from "./sessionItems.mjs";
...
export function getSessionCompactionState(session, state) {
...
export function bindSessionCompactionState(state, binding) {
    if (binding) {
        sessionCompactionStates.set(state, binding);
    }
}
...
export async function prepareResumedSessionWrite(session, state) {
    if (state._pendingSessionWrite !== undefined) {
        throw new UserError('A pending Session write must be reconciled before another append is prepared.');
    }
    if (!state._resumedSessionWriteInProgress) {
        throw new UserError('A resumed Session append requires exclusive ownership of its RunState.');
    }
    if (state._currentStep?.type !== 'next_step_interruption') {
        throw new UserError('Only interrupted tool work can prepare a resumed Session append.');
    }
    const sessionId = await session.getSessionId();
    if (sessionId.length === 0) {
        throw new UserError('Cannot prepare a Session append without a session ID.');
    }
    return {
        session,
        state,
...
export function acquireResumedSessionWriteOperation(state) {
    if (state._resumedSessionWriteInProgress) {
...
function checkpointPreparedResumedSessionWrite(options) {
    ...
    if (preparation.session !== session || preparation.state !== state) {
        throw new UserError('Resumed Session write preparation belongs to a different runtime operation.');
    }
    if (state._pendingSessionWrite !== undefined ||
    ...
    if (state._currentStep?.type !== 'next_step_run_again' &&
        ...
        throw new UserError('Only resumed tool work awaiting another model turn, approval, or terminal tool finalization can checkpoint a Session append.');
    }
    const pendingItems = {
    ...
    const reconstructedItems = getPendingSessionWriteAppendItems(state, pendingItems);
    if (reconstructedItems.length !== items.length ||
        !agentItemRangeMatches(reconstructedItems, items, 0)) {
        throw new UserError('Resumed Session write items do not match their canonical RunState owners.');
    }
    const pending = {
        phase: 'prepared',
        ...pendingItems,
        handoffInput: preparation.handoffInput,
        terminalToolFinalization: capturePendingSessionWriteTerminalFinalization(state),
    };
...
function promotePreparedSessionWrite(session, state, storedItems) {
    ...
    if (beforeItems.length !== storedItems.length ||
        ...
        throw new UserError('Session persistence comparison must preserve stored item boundaries.');
    }
    if (!sessionItemArraysMatch(combinedItems, expectedCombinedItems)) {
        throw new UserError('Session history changed before a prepared resumed write could be completed safely.');
    }
    const appendReady = {
        ...pending,
        phase: 'append_ready',
...
async function appendPreparedResumedSessionWrite(options) {
    ...
    if (sessionId !== preparation.sessionId) {
        throw new UserError('A prepared Session write belongs to a different session and cannot be resumed safely.');
    }
    ...
    await addSessionItems(session, getPendingSessionWriteAppendItems(state, pending), state._context, getSessionCompactionState(session, state));
}
export async function reconcilePendingSessionWriteBeforeRun(session, state, options) {
    const pending = state._pendingSessionWrite;
    ...
    if (options.operationAlreadyOwned === true &&
        ...
        throw new UserError('Pending Session write reconciliation lost exclusive RunState ownership.');
    }
    ...
    try {
        ...
        if (options.serverManagesConversation || session === undefined) {
            throw new UserError('A pending Session write requires the same ordinary local Session to resume safely.');
        }
        ...
        if (getEffectiveSessionReasoningItemIdPolicy(session, state) !==
            ...
            throw new UserError('Session persistence policy changed while a resumed write was pending.');
        }
        if (state._currentTurnSessionHistoryTransactionSessionId !== undefined &&
            !isSessionHistoryTransactionAwareSession(session)) {
            throw new UserError('A pending transaction-aware Session write requires the same transaction-aware local Session to resume safely.');
        }
        const appendItems = getPendingSessionWriteAppendItems(state, pending);
        if (pending.phase === 'append_ready') {
            ...
            if (pending.comparableAppendItems.length !== appendItems.length ||
                comparableAppendItems.length !== appendItems.length ||
                !sessionItemArraysMatch(comparableAppendItems, pending.comparableAppendItems)) {
                throw new UserError('RunState pending Session write comparison evidence does not match the runtime Session.');
            }
        }
        const sessionId = await session.getSessionId();
        if (sessionId !== pending.sessionId) {
            throw new UserError('A pending Session write belongs to a different session and cannot be resumed safely.');
        }
        const storedItems = await getSessionItems(session, state._context);
        ...
        if (sessionItemArraysMatch(currentItems, appendReady.beforeItems)) {
            ...
            throw new UserError('Session history changed while a resumed write was unacknowledged and cannot be reconciled safely.');
        }
...
function markPendingApprovedToolInputCompaction(state) {
    const pending = state._pendingSessionWrite;
    if (pending?.phase !== 'append_ready') {
        throw new UserError('A settled resumed Session append is missing its compaction authority.');
    }
...
async function completePendingApprovedToolInputCompaction(session, state) {
    ...
    if (getEffectiveSessionReasoningItemIdPolicy(session, state) !==
        pending.reasoningItemIdPolicy) {
        throw new UserError('Session persistence policy changed while compaction was pending.');
    }
    if ((await session.getSessionId()) !== pending.sessionId) {
        throw new UserError('A pending Session compaction belongs to a different session and cannot be resumed safely.');
    }
 ...
 */
export function mapOwnedInputItemsAfterContextProcessing(preparedItems, processedItems, ownedItems) {
...
export async function saveStreamInputToSession(session, sessionInputItems, runContext, compactionState) {
    if (!session) {
        return;
    }
...
export async function prepareInputItemsWithSession(input, session, sessionInputCallback, options, runContext, compactionState) {
...
async function persistRunItemsToSession(options) {
    ...
    if (resumedSessionWritePreparation &&
        ...
        if (getEffectiveSessionReasoningItemIdPolicy(session, state) !==
            ...
            throw new UserError('Session persistence policy changed after resumed tool execution.');
        }
    }
    ...
    if (itemsToSave.length === 0) {
        const pendingApprovedToolInputCompaction = hasPendingApprovedToolInputCompaction(state);
        if (pendingApprovedToolInputCompaction) {
            if (!runCompaction || compactionMode !== 'input') {
                throw new UserError('Pending Session compaction must settle before other persistence work.');
            }
            await completePendingApprovedToolInputCompaction(session, state);
            return;
        }
...
async function runCompactionOnSession(session, responseId, state, compactionMode) {
    ...
    const compactionResult = isOpenAIResponsesCompactionOwnershipAwareSession(session)
