import { ModelTimeoutError, UserError } from "../errors.mjs";
import { RequestUsage, Usage } from "../usage.mjs";
import { attachModelFailureUsage, consumeModelFailureUsage, createModelFailureUsageScope, } from "./usageTracking.mjs";
...
export function validateModelTimeoutMs(modelSettings) {
    ...
    if (!Number.isFinite(timeoutMs) ||
        ...
        throw new UserError(`modelSettings.timeoutMs must be a positive finite number less than or equal to ${MAX_MODEL_TIMEOUT_MS} when provided.`);
    }
    return timeoutMs;
}
...
export async function getResponseWithRetry(model, request, handlers = {}) {
    ...
    try {
        while (true) {
            ...
            const usageScope = createModelFailureUsageScope(attemptScope.request, failedAttemptUsage.report);
            try {
...
export async function* getStreamedResponseWithRetry(model, request, handlers = {}) {
    const maxRetries = request.modelSettings.retry?.maxRetries ?? 0;
    const retryPolicy = request.modelSettings.retry?.policy;
    const retryBackoff = request.modelSettings.retry?.backoff;
    const timeoutMs = validateModelTimeoutMs(request.modelSettings);
                ...
                for await (const event of model.getStreamedResponse(usageScope.request)) {
                    ...
                    if (event.type === 'response_done' &&
                        ...
                        yield {
                            ...
                            response: {
                                ...event.response,
                                usage: failedAttemptUsage.addTo(new Usage(event.response.usage)),
                            },
                        };
                        continue;
                    }
...
//# sourceMappingURL=modelRetry.mjs.map