...
exports.validateModelTimeoutMs = validateModelTimeoutMs;
...
function addFailedRetryAttemptsToUsage(usage, failedUsage) {
    if (failedUsage.length === 0) {
        return usage;
    }
    const inferredEndpoint = usage.requestUsageEntries?.[0]?.endpoint;
    const combinedUsage = new usage_1.Usage();
    for (const failedAttemptUsage of failedUsage) {
        combinedUsage.add(failedAttemptUsage ??
            new usage_1.Usage({
                requests: 1,
                requestUsageEntries: [
    ...
    combinedUsage.add(usage.requestUsageEntries?.length
        ? usage
        : new usage_1.Usage({
...
function combineKnownFailureUsage(failedUsage) {
    let combinedUsage;
    for (const usage of failedUsage) {
        if (!usage) {
            continue;
        }
        combinedUsage ??= new usage_1.Usage();
        combinedUsage.add(usage);
    }
    return combinedUsage;
}
function createFailedAttemptUsageTracker(onFailureUsage) {
    const failedUsage = [];
    let pendingUsage;
    const report = (usage) => {
        pendingUsage ??= new usage_1.Usage();
    ...
    return {
        ...
        record(error) {
            const usage = (0, usageTracking_1.consumeModelFailureUsage)(error);
            if (usage) {
                report(usage);
            }
            finishAttempt();
        ...
        addTo(usage) {
            ...
            const combinedUsage = addFailedRetryAttemptsToUsage(usage, onFailureUsage
        ...
        finishFailure(error) {
            const replacementUsage = (0, usageTracking_1.consumeModelFailureUsage)(error);
            if (replacementUsage) {
                report(replacementUsage);
            }
            const knownUsage = takeKnownUsage();
            if (knownUsage && !onFailureUsage) {
                (0, usageTracking_1.attachModelFailureUsage)(error, knownUsage);
            }
        },
        close() {
            failedUsage.length = 0;
...
function validateModelTimeoutMs(modelSettings) {
    ...
    if (!Number.isFinite(timeoutMs) ||
        ...
        throw new errors_1.UserError(`modelSettings.timeoutMs must be a positive finite number less than or equal to ${MAX_MODEL_TIMEOUT_MS} when provided.`);
    }
    return timeoutMs;
}
function createModelTimeoutError(timeoutMs, cause) {
    return new errors_1.ModelTimeoutError({ timeoutMs, cause });
}
function createModelAttemptScope(request, timeoutMs) {
    if (timeoutMs === undefined) {
        return {
            request,
            cleanup: () => { },
...
function isStatefulRequest(request) {
...
function normalizeRetryError(error, signal, providerAdvice) {
    const isModelTimeout = error instanceof errors_1.ModelTimeoutError;
    ...
    const normalized = {
        ...
        isAbort: !isModelTimeout && (Boolean(signal?.aborted) || isAbortLikeError(error)),
    };
    ...
    return {
        ...
        isAbort: !isModelTimeout &&
            (normalized.isAbort || providerNormalized?.isAbort === true),
    };
}
function createProviderRetryAuthority(providerAdvice) {
    const replaySafety = providerAdvice?.replaySafety;
    return Object.freeze({
        suggested: providerAdvice?.suggested,
        replaySafety: replaySafety === 'safe' || replaySafety === 'unsafe'
            ? replaySafety
            : 'unknown',
        responseStarted: providerAdvice?.responseStarted,
    });
}
function withTimeoutReplayAuthority(providerAdvice, timedOut) {
...
function requestMayHaveBeenAccepted(authority) {
    return (authority.replaySafety === 'unsafe' || authority.responseStarted === true);
}
function withProviderRetryAuthority(context, authority = createProviderRetryAuthority(context.providerAdvice)) {
    const existing = context[providerRetryAuthoritySymbol];
    if (existing) {
        return context;
    }
...
function getProviderRetryAuthority(context) {
    return (context[providerRetryAuthoritySymbol] ??
...
function withDelegableReplayVeto(decision) {
...
function isDelegableReplayVeto(decision) {
    return (isHardVeto(decision) &&
...
function withUnsafeReplayApproval(decision, approveUnsafeReplay) {
...
function resolveDelegableReplayVeto(veto, approving) {
...
function throwIfTimedModelCallParentAborted(signal, timeoutMs) {
    if (timeoutMs !== undefined) {
        throwIfAborted(signal);
    }
}
async function awaitWithTimedModelCallParentAbort(promise, signal, timeoutMs) {
...
async function evaluateRetry({ error, attempt, maxRetries, retryPolicy, retryBackoff, signal, stream, request, emittedVisibleEvent, emittedRawModelEvent, providerAdvice, allowUnsafeStreamReplayApproval, }) {
    ...
    any(...policies) {
        return async (inputContext) => {
            ...
            for (const policy of policies) {
                ...
                if (decision.retry) {
                    const approveUnsafeReplay = Boolean(firstRetryDecision?.approveUnsafeReplay ||
                    ...
                    firstRetryDecision = withUnsafeReplayApproval(firstRetryDecision, approveUnsafeReplay);
                    continue;
                }
...
async function getResponseWithRetry(model, request, handlers = {}) {
    ...
    const failedAttemptUsage = createFailedAttemptUsageTracker(handlers.onModelFailureUsage);
    let attempt = 1;
    try {
        while (true) {
            ...
            const usageScope = (0, usageTracking_1.createModelFailureUsageScope)(attemptScope.request, failedAttemptUsage.report);
            try {
                ...
                if (attemptScope.timedOut()) {
                    throw (attemptScope.timeoutError() ?? createModelTimeoutError(timeoutMs));
                }
                ...
                let providerAdvice;
                try {
                    providerAdvice = await awaitWithTimedModelCallParentAbort(getRetryAdvice(model, {
                        request,
                        error: caughtError,
                        stream: false,
                        attempt,
                ...
                providerAdvice = withTimeoutReplayAuthority(providerAdvice, attemptScope.timedOut());
                const authority = createProviderRetryAuthority(providerAdvice);
                ...
                let decision;
                try {
                    decision = await awaitWithTimedModelCallParentAbort(evaluateRetry({
                        error,
                        attempt,
                        maxRetries,
                        retryPolicy,
...
async function* getStreamedResponseWithRetry(model, request, handlers = {}) {
    const maxRetries = request.modelSettings.retry?.maxRetries ?? 0;
    const retryPolicy = request.modelSettings.retry?.policy;
    const retryBackoff = request.modelSettings.retry?.backoff;
    const timeoutMs = validateModelTimeoutMs(request.modelSettings);
    ...
    try {
        while (true) {
            ...
            const usageScope = (0, usageTracking_1.createModelFailureUsageScope)(attemptScope.request, failedAttemptUsage.report);
            try {
                for await (const event of model.getStreamedResponse(usageScope.request)) {
                    throwIfTimedModelCallParentAborted(request.signal, timeoutMs);
                    if (attemptScope.timedOut()) {
                        throw (attemptScope.timeoutError() ?? createModelTimeoutError(timeoutMs));
                    }
                    ...
                    if (event.type === 'response_done' &&
                        ...
                        yield {
                            ...
                            response: {
                                ...event.response,
                                usage: failedAttemptUsage.addTo(new usage_1.Usage(event.response.usage)),
                            },
                        };
                        continue;
                    }
