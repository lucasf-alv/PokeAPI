...
import { isGpt5OrNewerReasoningModel, isGpt5OrNewerDefault, } from "../defaultModel.mjs";
export { mergeModelSettings } from "./modelSettingsMerge.mjs";
const hasReasoningModelSettings = (settings) => {
 ...
 */
export function adjustModelSettingsForLegacyModel(explicitlyModelSet, agentModelSettings, modelSettings, resolvedModelName) {
    // Only a resolved string selection establishes model identity.
    const isLegacyModel = typeof resolvedModelName === 'string' &&
        !isGpt5OrNewerReasoningModel(resolvedModelName);
    const hasReasoningSettings = hasReasoningModelSettings(agentModelSettings) ||
        hasReasoningModelSettings(modelSettings);
    if (isGpt5OrNewerDefault() &&
        explicitlyModelSet &&
        isLegacyModel &&
...
function stripReasoningModelSettings(modelSettings) {
