...
exports.invalidateOutputItemNormalization = invalidateOutputItemNormalization;
...
function invalidateOutputItemNormalization(items) {
    for (const item of items) {
...
function extractOutputItemsFromRunItems(items, reasoningItemIdPolicy) {
    return items
        ...
        .map((item) => {
        const normalizationKey = item.type === 'reasoning_item' &&
            shouldOmitReasoningItemIds(reasoningItemIdPolicy)
            ? 'omit'
            : 'preserve';
        const cached = normalizedOutputItemByRunItem.get(item)?.[normalizationKey];
        if (cached) {
            return cached;
        }
        const withoutOutputOnlyMetadata = stripOutputOnlyCreatedBy(item.rawItem);
        const withoutNullStatusItem = withoutNullStatus(withoutOutputOnlyMetadata);
        const normalizedItem = normalizationKey === 'omit'
            ? stripReasoningItemIdForPolicy(withoutNullStatusItem, reasoningItemIdPolicy)
...
function stripOutputOnlyCreatedBy(item) {
    if (!item || typeof item !== 'object') {
        return item;
    }
    const candidate = item;
    const hasTopLevelCreatedBy = Object.prototype.hasOwnProperty.call(candidate, 'created_by');
    const output = candidate.output;
    const hasShellOutputCreatedBy = candidate.type === 'shell_call_output' &&
    ...
    const normalized = withoutCreatedBy;
    if (hasShellOutputCreatedBy) {
        normalized.output = output.map((entry) => {
            if (entry === null ||
                typeof entry !== 'object' ||
                !Object.prototype.hasOwnProperty.call(entry, 'created_by')) {
                return entry;
            }
            const { created_by: _chunkCreatedBy, ...withoutChunkCreatedBy } = entry;
 ...
 */
function getRunOutput(generatedItems, reasoningItemIdPolicy) {
    return getContinuationOutputItems(generatedItems.filter((item) => item.type !== 'input_item'), reasoningItemIdPolicy);
}
/**
