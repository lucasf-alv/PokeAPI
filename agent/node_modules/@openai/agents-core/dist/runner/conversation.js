...
function getServerItemKey(item) {
    return (0, items_2.getAgentInputItemKey)(item.type === 'tool_search_output'
        ? (0, toolSearchAttribution_1.attributeToolSearchOutput)(item, undefined)
        : item);
}
/**
...
class ServerConversationTracker {
    ...
    primeFromState({ originalInput, generatedItems, modelResponses, }) {
        ...
        for (const [responseIndex, response] of modelResponses.entries()) {
            ...
            for (const item of response.output) {
                if (item && typeof item === 'object') {
                    ...
                    serverItemKeys.add(getServerItemKey(item));
                }
            }
        }
        if (hasResponses) {
        ...
        if (hasResponses) {
            for (const item of generatedItems) {
                ...
                const rawItemKey = getServerItemKey(rawItem);
     ...
     */
    trackServerItems(modelResponse, processedItems = []) {
        if (!modelResponse) {
            return;
        }
        ...
        const searchOutputs = processedItems.filter((item) => item instanceof items_1.RunToolSearchOutputItem);
        if (searchOutputs.length > 0) {
            const serverSearchKeys = new Set(modelResponse.output
