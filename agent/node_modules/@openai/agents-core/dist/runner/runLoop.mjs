...
import { getFunctionToolStateKeyForCall } from "../toolIdentity.mjs";
import { getServerConversationOwner } from "./conversation.mjs";
import { preflightToolInvocations, resolveInterruptedTurn, resolveTurnAfterModelResponse, } from "./turnResolution.mjs";
export function isAcceptedResponseCheckpoint(state) {
...
function assertAcceptedResponseServerOwnership(state) {
    if (getServerConversationOwner(state._conversationId, state._previousResponseId)) {
        return;
    }
    throw new UserError('Accepted model response state requires exactly one server-managed conversation owner', state);
}
export function assertAcceptedResponseContinuationAuthority(state, requestedConversationId, requestedPreviousResponseId) {
    ...
    if ((requestedConversationId !== undefined &&
        ...
        throw new UserError('Accepted model response state cannot change its server-managed conversation owner', state);
    }
}
export function markAcceptedResponseProcessingStarted(state) {
    const currentStep = state._currentStep;
...
export function markAcceptedResponseFinalizationStarted(state) {
    const currentStep = state._currentStep;
    ...
    if (currentStep.localFinalizationStarted === true) {
        throw new UserError('An accepted model response has unfinished local finalization and cannot be resumed safely; start a new run instead', state);
    }
 ...
 */
export async function resumeAcceptedModelResponse(options) {
    const { state } = options;
    if (!state._lastTurnResponse || !state._lastProcessedResponse) {
        throw new UserError('An accepted model response could not be processed; start a new run instead of retrying it', state);
    }
    const suppressedToolCalls = preflightToolInvocations(state._currentAgent, state, state._lastProcessedResponse);
    if (state._currentStep?.type === 'next_step_interruption' &&
        ...
        throw new UserError('An accepted model response has unfinished local tool work and cannot be resumed safely; start a new run instead', state);
    }
...
export async function resumeInterruptedTurn(options) {
    ...
    const turnResult = await resolveInterruptedTurn(state._currentAgent, state._originalInput, state._generatedItems, state._lastTurnResponse, state._lastProcessedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, validateHandoffAgent, preparation
        ? (input) => {
