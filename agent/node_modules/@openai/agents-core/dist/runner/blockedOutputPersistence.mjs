...
import { RunHandoffOutputItem, RunToolCallItem, RunToolCallOutputItem, RunToolSearchCallItem, RunToolSearchOutputItem, } from "../items.mjs";
import { getToolCallName, getToolCallNamespace, getToolCallQualifiedName, getFunctionToolLookupKeyForCall, matchesFunctionToolName, } from "../toolIdentity.mjs";
import { getToolSearchExecution, getToolSearchMatchKey, getToolSearchOutputReplacementKey, getToolSearchProviderCallId, } from "../tooling.mjs";
import { FunctionCallItem as FunctionCallItemSchema, FunctionCallResultItem as FunctionCallResultItemSchema, } from "../types/protocol.mjs";
import { Usage } from "../usage.mjs";
import { toSmartString } from "../utils/smartString.mjs";
import { getSerializedOutputGuardrailResults, sanitizeBlockedOutputGuardrailResults, sanitizeBlockedToolOutputGuardrailResults, replaceSanitizedOutputGuardrailMessages, } from "./guardrails.mjs";
import { invalidateOutputItemNormalization } from "./items.mjs";
import { getToolResultCorrelationForCall, getToolResultCorrelationForResult, getToolResultCorrelationKey, } from "./toolResultCorrelation.mjs";
import { addLoadedToolNamesFromToolSearchOutput } from "./toolSearch.mjs";
import { OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT } from "./outputGuardrailBlockedMessage.mjs";
const currentResponseToolOutputGuardrailResultStarts = new WeakMap();
export function captureCurrentResponseToolOutputGuardrailResultStart(state, overwrite) {
    if (overwrite || !currentResponseToolOutputGuardrailResultStarts.has(state)) {
        currentResponseToolOutputGuardrailResultStarts.set(state, state._toolOutputGuardrailResults.length);
    }
}
export function getCurrentResponseToolOutputGuardrailResultStart(state) {
    return currentResponseToolOutputGuardrailResultStarts.get(state);
}
export function restoreCurrentResponseToolOutputGuardrailResultStart(state, startIndex) {
    currentResponseToolOutputGuardrailResultStarts.set(state, startIndex);
}
function currentCompletedToolRuns(state) {
    const responseCalls = getResponseOutput(currentResponse(state))?.filter((item) => item.type === 'function_call');
    ...
    return processedRuns.slice(-responseCalls.length).filter((run, index) => {
        try {
            return (JSON.stringify(buildCanonicalFunctionCall(run.toolCall)) ===
...
function currentTerminalToolRuns(state) {
    ...
    if (typeof behavior === 'object' &&
        !completedRuns.some((run) => behavior.stopAtToolNames.some((toolName) => matchesFunctionToolName(run.tool, toolName)))) {
        return [];
    }
    return completedRuns;
}
function currentDeclarativeTerminalToolOutput(state) {
    const behavior = state._currentAgent.toolUseBehavior;
    if (behavior === 'run_llm_again' ||
    ...
    const selectedRun = behavior === 'stop_on_first_tool'
        ? completedRuns[0]
        : completedRuns.find((run) => behavior.stopAtToolNames.some((toolName) => matchesFunctionToolName(run.tool, toolName)));
...
export function hasTerminalToolOutputSource(state) {
    return (state._finalOutputSource === 'tool_result' ||
        (state._finalOutputSource === undefined &&
            state._serializedCurrentStep === state._currentStep &&
            currentTerminalToolRuns(state).length > 0));
}
export function sanitizeBlockedTerminalToolOutput(state, outputGuardrailResultStart, completedOutputGuardrailTripwireResult, observedOutputGuardrailResults, tripwire, ownedOutputGuardrailResults, resolveBlockedMessage, signal, onBlockedOutput) {
    if (!hasTerminalToolOutputSource(state) ||
        !completedOutputGuardrailTripwireResult) {
        return false;
    }
    onBlockedOutput?.();
    redactBlockedResponseToolOutputs(state);
    const sanitizedOutputGuardrailResults = sanitizeBlockedOutputGuardrailResults(state, outputGuardrailResultStart, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT, observedOutputGuardrailResults, tripwire, ownedOutputGuardrailResults);
    sanitizeBlockedToolOutputGuardrailResults(state, currentResponseToolOutputGuardrailResultStarts.get(state) ?? 0, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT);
    ...
    return awaitBlockedMessageWithAbort(resolveBlockedMessage(guardrailName), signal)
        .then((blockedMessage) => {
        signal?.throwIfAborted();
        if (typeof blockedMessage !== 'string' ||
        ...
        replaceSanitizedOutputGuardrailMessages(state, sanitizedOutputGuardrailResults, blockedMessage, tripwire);
...
function awaitBlockedMessageWithAbort(promise, signal) {
    if (!signal) {
        return promise;
    }
    signal.throwIfAborted();
...
export function shouldDeferInterruptedSessionItems(state, hasOutputGuardrail) {
    return (state._currentStep?.type === 'next_step_interruption' &&
        hasOutputGuardrail &&
        state._currentAgent.toolUseBehavior !== 'run_llm_again' &&
        hasOutputBearingApprovalCheckpoint(state));
}
export function assertResumedSessionOutputGuardrailSafety(state, session, hasOutputGuardrail) {
    if (hasOutputGuardrail &&
        state._serializedCurrentStep === state._currentStep &&
        state._currentStep?.type === 'next_step_final_output' &&
        state._finalOutputSource === undefined &&
        currentCompletedToolRuns(state).length > 0 &&
        !hasTerminalToolOutputSource(state)) {
        throw new UserError('Cannot resume this serialized terminal output because terminal tool output provenance could not be verified after the tool behavior changed. Continue the live RunState or start a new run from safe input.');
    }
    const shouldDefer = shouldDeferInterruptedSessionItems(state, hasOutputGuardrail);
    ...
    if (state._serializedCurrentStep !== undefined && shouldDefer) {
        ...
        if (!responseOutput ||
            ...
            throw new UserError(`Cannot resume this serialized output-bearing approval checkpoint because current-response provenance was not preserved (${reasons.join(', ')}). Start a new run from safe input.`);
        }
    }
    ...
    const currentCallIds = new Set((state._lastProcessedResponse?.functions ?? []).map((run) => run.toolCall.callId));
    const currentStart = restoredSelection
        ? state._generatedItems.length - restoredSelection.primary.length
        : state._generatedItems.findIndex((item) => item instanceof RunToolCallItem &&
            item.rawItem.type === 'function_call' &&
            currentCallIds.has(item.rawItem.callId));
    const firstOutputIndex = state._generatedItems.findIndex((item, index) => index >= currentStart && item instanceof RunToolCallOutputItem);
    if (currentStart >= 0 &&
        firstOutputIndex === state._currentTurnPersistedItemCount &&
        state._generatedItems
            .slice(currentStart, firstOutputIndex)
            .every((item) => 'rawItem' in item &&
            item.rawItem.type === 'function_call' &&
            currentCallIds.has(item.rawItem.callId))) {
        return;
    }
    throw new UserError('Cannot resume an output-bearing approval checkpoint with a Session while output guardrails are enabled because the persisted response ownership cannot be proven. Start a new run from safe input.');
}
...
function hasOnlyCompletedStatusEvidence(itemStatus, providerStatus, allowMissing) {
    if (itemStatus === undefined && providerStatus === undefined) {
        return allowMissing;
    }
    return ((itemStatus === undefined || itemStatus === 'completed') &&
        (providerStatus === undefined || providerStatus === 'completed'));
}
function supportsLocalExecutionProvenance(type) {
    return (type === 'function_call_result' ||
        type === 'shell_call_output' ||
        type === 'computer_call_result' ||
        type === 'apply_patch_call_output');
}
function getBlockedPairKind(item, role) {
    if (role === 'call') {
...
function classifyBlockedToolRecord(item) {
    ...
    switch (type) {
        ...
        case 'computer_call_result': {
            ...
            const providerStatus = providerData && typeof providerData === 'object'
                ? providerData.status
                : undefined;
        ...
        case 'shell_call_output': {
            role = 'result';
            terminal = hasOnlyCompletedStatusEvidence(status, getProviderStatus(rawItem), true);
            break;
        }
...
function getBlockedRunItemAgent(item) {
    return item instanceof RunHandoffOutputItem ? item.sourceAgent : item.agent;
}
function isReplaySafeTerminalHostedToolCall(item) {
    const rawItem = item.rawItem;
    const providerData = rawItem?.type === 'hosted_tool_call' &&
        rawItem.providerData &&
        typeof rawItem.providerData === 'object'
        ? rawItem.providerData
        : undefined;
    const itemStatus = rawItem?.type === 'hosted_tool_call' ? rawItem.status : undefined;
...
function collectBlockedToolSearchPairs(items) {
...
function toolSearchOutputLoadsRetainedCall(output, call) {
 ...
 */
export function selectRunItemIndexesForBlockedOutput(items, unpersistedStartIndex = 0) {
    ...
    for (const [index, item] of items.entries()) {
        if (isReplaySafeTerminalHostedToolCall(item)) {
            standaloneCalls.push({
                index,
                programCallerId: getProgramCallerId(item.rawItem),
    ...
    const retainedCallIndexes = new Set();
    const directlyRetainedCallIndexes = new Set();
    const retainedResultIndexByCallIndex = new Map();
    const programOwnerIndexByCallIndex = new Map();
    const retainCallWithProgramOwner = (call) => {
        if (call.programCallerId !== undefined) {
            const ownerIndexes = programOwnerIndexesByAgent.get(call.agent)?.get(call.programCallerId) ??
                [];
    ...
    for (const retainedCallIndex of [...retainedCallIndexes]) {
        const retainedCall = items[retainedCallIndex];
        const rawItem = retainedCall.rawItem;
        if (!rawItem || !isToolSearchDependentRetainedCall(rawItem)) {
            continue;
        }
        const precedingOccurrences = toolSearchPairs.filter((pair) => pair.agent === retainedCall.agent &&
            pair.outputIndex < retainedCallIndex);
        const hasMatchingOutput = precedingOccurrences.some((pair) => toolSearchOutputLoadsRetainedCall(pair.output, rawItem));
        if (!hasMatchingOutput) {
            continue;
        }
        const latestByReplacementKey = new Map();
        const additiveOccurrences = [];
        ...
        const supplier = [
            ...
            .filter((occurrence) => occurrence.valid &&
    ...
    const neededProgramOwnerIndexes = new Set([...programOwnerIndexByCallIndex]
    ...
    for (const ownerIndex of programOwnerIndexByCallIndex.values()) {
        if (!directlyRetainedCallIndexes.has(ownerIndex) &&
...
export function hasBlockedOutputExecutionEffect(items, unpersistedStartIndex = 0) {
    return (selectRunItemIndexesForBlockedOutput(items, unpersistedStartIndex).length >
        0 ||
        items
            .slice(unpersistedStartIndex)
            .some((item) => item instanceof RunToolCallOutputItem &&
            item.executionStatus === 'executed'));
}
export function buildRunItemPersistencePlan(options) {
    const { items, alreadyPersistedCount, currentDeferredIndexes, outputBlocked, canUseHistoryTransactions, blockedSnapshot, } = options;
    const newRunItemIndexes = Array.from({ length: Math.max(0, items.length - alreadyPersistedCount) }, (_value, offset) => alreadyPersistedCount + offset);
    if (outputBlocked) {
        if (blockedSnapshot) {
            const snapshotPersistedCount = canUseHistoryTransactions
                ? alreadyPersistedCount
                : blockedSnapshot.alreadyPersistedCount;
            const persistedSnapshotItems = Math.max(0, snapshotPersistedCount - blockedSnapshot.startIndex);
            return {
        ...
        const retainedIndexes = selectRunItemIndexesForBlockedOutput(items, alreadyPersistedCount);
    ...
    if (!canUseHistoryTransactions) {
        throw new UserError('Cannot persist accepted output from this RunState because its blocked output was saved by a transaction-aware session. Resume with the same transaction-aware session.');
    }
    ...
    if (runItemsToReplace.length === 0) {
        ...
        if (anchorIndex < 0) {
            throw new UserError('Cannot persist accepted output because the blocked session suffix has no retained anchor.');
        }
...
function buildCanonicalFunctionCall(rawItem) {
...
export function buildBlockedToolOutputRawItem(rawItem, blockedMessage = OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT) {
...
function copyResponseString(response, key) {
    try {
...
function cloneBlockedResponse(response, output) {
    let usage = new Usage();
    try {
...
function currentResponse(state) {
    return state._lastTurnResponse;
}
function functionCallKey(item) {
...
function functionResultKey(item) {
...
function functionRunItemSignature(item) {
    try {
...
function functionRunItemSequenceIsPrefix(items, prefix) {
...
function boundedCurrentFunctionResponseStart(generated, responseOutput) {
    ...
    const calls = suffix.filter((item) => item instanceof RunToolCallItem && item.rawItem.type === 'function_call');
    const results = suffix.filter((item) => item instanceof RunToolCallOutputItem &&
        item.rawItem.type === 'function_call_result');
    if (calls.length !== responseKeys.length ||
        results.length !== responseKeys.length ||
        calls.some((item, index) => functionCallKey(item.rawItem) !== responseKeys[index]) ||
        results.some((item, index) => functionResultKey(item.rawItem) !== responseKeys[index])) {
        return undefined;
    }
    return start;
}
function restoredCurrentRunItemSelection(state, responseOutput) {
    if (state._serializedCurrentStep !== state._currentStep ||
    ...
    const responseKeys = responseOutput.map(functionCallKey);
    if (getCurrentResponseToolOutputGuardrailResultStart(state) === undefined ||
        processed.length === 0 ||
        processed.length !== responseKeys.length ||
        responseKeys.some((key) => key === undefined) ||
        processed.some((item, index) => !(item instanceof RunToolCallItem) ||
            item.rawItem.type !== 'function_call' ||
            functionCallKey(item.rawItem) !== responseKeys[index])) {
        const fallbackStart = Math.min(state._currentTurnPersistedItemCount, generated.length);
        const fallback = generated.slice(fallbackStart);
        return { primary: fallback, aliases: fallback, proven: false };
    }
    const matchingStarts = [];
    for (let start = 0; start <= generated.length - processed.length; start += 1) {
    ...
    const processedEnd = start === undefined ? undefined : start + processed.length;
    const interruptionIndexes = state._currentStep.data.interruptions.map((interruption) => generated.flatMap((item, index) => item === interruption ? [index] : []));
    if (start === undefined ||
        interruptionIndexes.some((indexes) => indexes.length !== 1 || indexes[0] < (processedEnd ?? Infinity))) {
        const fallbackStart = Math.min(state._currentTurnPersistedItemCount, generated.length);
        const fallback = generated.slice(fallbackStart);
        return { primary: fallback, aliases: fallback, proven: false };
    }
...
function currentRunItemSelection(state, responseOutput) {
    const restoredSelection = restoredCurrentRunItemSelection(state, responseOutput);
    ...
    const processed = state._lastProcessedResponse?.newItems ?? [];
    const ownsResponseItem = (item) => {
        try {
            return ('rawItem' in item && responseObjects.has(item.rawItem));
        }
        catch {
            return false;
        }
    ...
    const processedOwnerStart = generated.findIndex((item) => processedItems.has(item));
    const completedInvocationOwners = new Set((state._lastProcessedResponse?.functions ?? []).flatMap((run) => state._completedToolInvocationEvidence
        .get(state._currentAgent)
        ?.get(run.toolCall.callId)?.items ?? []));
    const completedInvocationOwnerStart = generated.findIndex((item) => completedInvocationOwners.has(item));
    const boundedCurrentStart = boundedCurrentFunctionResponseStart(generated, responseOutput);
    ...
    if (start >= 0) {
        ...
        const currentProcessedCalls = new Set((state._lastProcessedResponse?.functions ?? []).map((run) => run.toolCall));
        let currentProcessedStart = processed.findIndex(ownsResponseItem);
        if (currentProcessedStart < 0) {
            currentProcessedStart = processed.findIndex((item) => primary.includes(item));
        }
        if (currentProcessedStart < 0) {
            currentProcessedStart = processed.findIndex((item) => item instanceof RunToolCallItem &&
                item.rawItem.type === 'function_call' &&
    ...
    if (state._currentTurnBlockedSessionStartIndex !== undefined) {
        const blockedStart = Math.min(state._currentTurnBlockedSessionStartIndex, generated.length);
        return {
...
function buildCurrentFunctionPairPlan(state, responseOutput, selection, blockedMessage = OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT) {
    if (!selection.proven)
        return undefined;
    try {
        ...
        for (const [index, item] of selection.primary.entries()) {
            if (item instanceof RunToolCallItem &&
                ...
                if (!key || callIndexes.has(key))
                    return undefined;
                callKeys.push(key);
                callIndexes.set(key, index);
                sanitizedItems.push(new RunToolCallItem(buildCanonicalFunctionCall(item.rawItem), item.agent, item.functionToolStateKey));
                continue;
            }
            if (item instanceof RunToolCallOutputItem &&
                item.rawItem.type === 'function_call_result') {
                const key = functionResultKey(item.rawItem);
                if (!key || resultIndexes.has(key))
                    return undefined;
                resultKeys.push(key);
                resultIndexes.set(key, index);
                sanitizedItems.push(new RunToolCallOutputItem(buildBlockedToolOutputRawItem(item.rawItem, blockedMessage), item.agent, blockedMessage, undefined, item.executionStatus));
                continue;
            }
            return undefined;
        }
        if (callKeys.length !== responseKeys.length ||
            ...
            responseKeys.some((key) => callIndexes.get(key) === undefined ||
        ...
        let currentProcessedStart = processed.findIndex((item) => aliases.has(item));
        if (currentProcessedStart < 0 &&
        ...
        if (currentProcessedStart >= 0) {
            ...
            if (currentProcessed.some((item) => !aliases.has(item)) ||
            ...
            for (const [index, item] of currentProcessed.entries()) {
                ...
                if (replacement instanceof RunToolCallItem &&
                    item instanceof RunToolCallItem) {
                    replacements.set(item, new RunToolCallItem(replacement.rawItem, item.agent, replacement.functionToolStateKey));
                }
                else if (replacement instanceof RunToolCallOutputItem &&
                    item instanceof RunToolCallOutputItem) {
                    replacements.set(item, new RunToolCallOutputItem(replacement.rawItem, item.agent, blockedMessage, undefined, replacement.executionStatus));
                }
                else {
                    return undefined;
                }
        ...
        return {
            responseOutput: sanitizedItems.flatMap((item) => item instanceof RunToolCallItem && item.rawItem.type === 'function_call'
                ? [item.rawItem]
                : []),
            replacements,
            sanitizedItems,
...
function replaceRunItems(state, replacements, responseOutput, canonicalResponseOutput) {
    const rawReplacements = new Map();
    for (const [item, replacement] of replacements) {
        if ('rawItem' in item) {
            rawReplacements.set(item.rawItem, replacement && 'rawItem' in replacement
                ? replacement.rawItem
                : undefined);
        }
    }
    ...
    const replace = (items) => items.flatMap((item) => {
        if (!replacements.has(item))
            return [item];
    ...
    if (state._lastProcessedResponse) {
        ...
        const replaceToolActions = (actions, allowRestoredCurrentPosition = false) => actions?.flatMap((action, index) => {
            const currentIndex = index - (actions.length - canonicalResponseOutput.length);
            if (!rawReplacements.has(action.toolCall) &&
                ...
                rawReplacements.set(action.toolCall, canonicalResponseOutput[currentIndex]);
            }
            if (!rawReplacements.has(action.toolCall))
                return [action];
        ...
        if (processed.functionToolsNotFound) {
            processed.functionToolsNotFound = replaceToolActions(processed.functionToolsNotFound);
        }
        if (processed.computerActions) {
            processed.computerActions = replaceToolActions(processed.computerActions);
        }
        if (processed.shellActions) {
            processed.shellActions = replaceToolActions(processed.shellActions);
        }
        if (processed.applyPatchActions) {
            processed.applyPatchActions = replaceToolActions(processed.applyPatchActions);
        }
    }
...
function replaceCurrentResponse(state, response, output) {
...
function markBlockedState(state, blockedMessage) {
    if (state._currentStep?.type === 'next_step_final_output') {
...
export function redactBlockedResponseToolOutputs(state, blockedMessage = OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT) {
    ...
    const plan = response && responseOutput
        ? buildCurrentFunctionPairPlan(state, responseOutput, selection, blockedMessage)
        : undefined;
    if (!plan) {
        const replacements = new Map();
        for (const item of selection.aliases)
            replacements.set(item, undefined);
        replaceRunItems(state, replacements, responseOutput, []);
        if (response)
            replaceCurrentResponse(state, response, []);
        markBlockedState(state, blockedMessage);
        return selection.aliases.length > 0 || response !== undefined;
    }
    replaceRunItems(state, plan.replacements, responseOutput, plan.responseOutput);
    replaceCurrentResponse(state, response, plan.responseOutput);
    markBlockedState(state, blockedMessage);
    return plan.replacements.size > 0 || plan.responseOutput.length > 0;
}
export function getBlockedOutputSessionSnapshotRunItems(state) {
    const responseOutput = getResponseOutput(currentResponse(state));
    if (!responseOutput)
        return [];
    const selection = currentRunItemSelection(state, responseOutput);
    const blockedMessage = state._currentStep?.type === 'next_step_final_output' &&
    ...
    const plan = buildCurrentFunctionPairPlan(state, responseOutput, selection, blockedMessage);
    return plan?.sanitizedItems ?? [];
}
export function isCanonicalBlockedOutputPayload(item) {
    return isCanonicalBlockedOutputPayloadForMessage(item, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT);
}
function isCanonicalBlockedOutputPayloadForMessage(item, blockedMessage) {
    if (item?.type !== 'function_call_result')
        return false;
    try {
...
function getCanonicalSerializedOutputGuardrailBlockedMessage(state) {
    if (state._serializedCurrentStep !== state._currentStep ||
        state._currentStep?.type !== 'next_step_final_output' ||
        typeof state._currentStep.output !== 'string' ||
        state._currentStep.output.length === 0 ||
    ...
    for (const [item, canonicalItem] of plan.replacements) {
        if (item instanceof RunToolCallItem &&
            ...
            if (JSON.stringify(item.rawItem) !== JSON.stringify(canonicalItem.rawItem)) {
                return undefined;
            }
            continue;
        }
...
export function normalizeSerializedOutputGuardrailBlockedMessage(state) {
    const blockedMessage = getCanonicalSerializedOutputGuardrailBlockedMessage(state);
    if (!blockedMessage ||
        !state._outputGuardrailResults.some((result) => result.agent === state._currentAgent &&
            result.agentOutput === blockedMessage &&
            result.output.tripwireTriggered === true &&
            result.output.outputInfo === undefined)) {
        return;
    }
    const ownedResults = getSerializedOutputGuardrailResults(state, blockedMessage);
    // A saved verdict only identifies data to neutralize; current guards must rerun.
    redactBlockedResponseToolOutputs(state);
    replaceSanitizedOutputGuardrailMessages(state, ownedResults, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT);
}
export function hasOutputBearingApprovalCheckpoint(state) {
    const responseOutput = getResponseOutput(currentResponse(state));
    if (!responseOutput)
        return true;
    if (responseOutput.some((item) => item.type !== 'function_call'))
        return true;
    const selection = currentRunItemSelection(state, responseOutput);
    if (!selection.proven)
        return true;
    return selection.primary.some((item) => item instanceof RunToolCallOutputItem ||
        (item instanceof RunToolCallItem &&
            item.rawItem.type === 'hosted_tool_call'));
}
//# sourceMappingURL=blockedOutputPersistence.mjs.map