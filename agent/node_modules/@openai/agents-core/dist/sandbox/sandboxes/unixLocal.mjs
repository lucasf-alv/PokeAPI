import { getRunStateSessionTrustedConfig } from "../internal/sessionStateTrust.mjs";
...
import { probeSandboxDirectoryExists } from "../shared/pathProbe.mjs";
import { cloneManifest, normalizeRelativePath } from "../manifest.mjs";
...
import { assertMountCredentialsRebound, assertHostPathGrantsRebound, mergeManifestEntryDelta, mergeManifestDelta, sanitizeEnvironmentForPersistence, serializeMountCredentialRedactionMetadata, serializeManifest, } from "./shared/manifestPersistence.mjs";
import { assertExistingMountTopologyPreserved, assertSandboxSessionStateUsable, assertSandboxStateGenerationUnchanged, captureLiveMountCredentialAuthority, captureSandboxStateGeneration, validateMountCredentialBoundaries, withExclusiveSandboxManifestMutation, } from "../mountSecurity.mjs";
import { isHostPathWithinRoot, relativeHostPathEscapesRoot, } from "../shared/hostPath.mjs";
import { MAX_VIEW_IMAGE_BYTES, imageOutputFromBytes } from "../shared/media.mjs";
import { UnixLocalFiles, preparedFileIO, } from "./shared/unixLocalFiles.mjs";
...
export class UnixLocalSandboxSession {
    ...
    createEditor(runAs) {
        ...
        return new UnixLocalSandboxEditor(this, (operation) => this.applyFileOperation(operation, runAs));
    }
    supportsPty() {
        return true;
    }
    ...
    async viewImage(args) {
        ...
        const bytes = await this.files
            ...
            if (error.code === 'EFBIG')
                throw new UserError(`Image file exceeds the 10 MB limit: ${args.path}`);
            throw error;
        });
    ...
    async pathExists(path, runAs) {
        ...
        return JSON.parse((await this.files.run({
            operation: 'exists',
    ...
    async directoryExists(path, runAs) {
        ...
        if (this.files.backend === 'node') {
            return probeSandboxDirectoryExists({
                path: resolvedPath,
                runCommand: (command) => runSandboxProcess('/bin/sh', ['-c', command], {
                    timeoutMs: RUN_AS_LOOKUP_TIMEOUT_MS,
                    ...(identity ? { uid: identity.uid, gid: identity.gid } : {}),
                }),
            });
        }
        return JSON.parse((await this.files.run({ operation: 'directoryExists', path: resolvedPath }, { identity })).toString('utf8'));
    }
    async readFile(args) {
        this.assertSessionUsable();
        await this.resolveFilesystemRunAs(args.runAs);
    ...
    async listDir(args) {
        ...
        const entries = JSON.parse((await this.files.run({
            operation: 'list',
            path: this.resolveFilesystemPath(args.path),
        })).toString('utf8'));
    ...
    async materializeEntry(args) {
        ...
        return await withExclusiveSandboxManifestMutation(this.state, async () => this.materializeEntryExclusive({ ...args, entry }));
    }
    async materializeEntryExclusive(args) {
        const logicalPath = this.resolveLogicalPath(args.path);
        const nextManifest = mergeManifestEntryDelta(this.state.manifest, logicalPath, args.entry);
    ...
    async applyManifest(manifest, runAs) {
        ...
        return await withExclusiveSandboxManifestMutation(this.state, async () => this.applyManifestExclusive(manifestSnapshot, runAs));
    }
    async applyManifestExclusive(manifest, runAs) {
    ...
    resolveFilesystemPath(path, options = {}, mode = 'content') {
    ...
    resolveHostPath(path, options, mode) {
        this.assertSessionUsable();
        ...
        if (resolved.grant) {
            ...
            const validated = validateResolvedHostPath({
                path,
                resolvedPath: resolve(root, relative(resolved.grant.path, resolved.path)),
                allowedRoot: root,
        ...
        const mountPath = this.resolveLocalBindMountPath(workspaceRelativePath, path, options, mode);
        if (mountPath) {
            return mountPath;
        }
        ...
        for (const path of paths) {
            if (!this.filesystemRoots.has(path)) {
                try {
                    ...
                    this.filesystemRoots.set(path, new UserError(`Sandbox filesystem root is unavailable: ${path}`));
                }
            }
        }
    }
    ...
    async applyFileOperation(operation, runAs) {
        this.assertSessionUsable();
        const owner = await this.resolveFilesystemRunAs(runAs);
        const path = this.resolveFilesystemPath(operation.path, { forWrite: true }, operation.type === 'delete_file' ? 'entry' : 'content');
        if (operation.type === 'create_file') {
            ...
            const destination = operation.moveTo
                ...
                : path;
            const unlinkPath = operation.moveTo &&
                this.resolveSandboxPath(operation.moveTo, { forWrite: true }) !==
                    this.resolveSandboxPath(operation.path, { forWrite: true })
                ? this.resolveFilesystemPath(operation.path, { forWrite: true }, 'entry')
                : undefined;
            await this.files.run({ operation: 'update', path, destination, unlinkPath, owner }, { update: (current) => applyDiff(current, operation.diff) });
        }
        else {
    ...
    resolveLocalBindMountPath(workspaceRelativePath, path, options, mode) {
...
export class UnixLocalSandboxClient {
    ...
    async resume(state, options = {}) {
        ...
        const files = new UnixLocalFiles(currentOptions?.fileIOProtection ?? this.options.fileIOProtection);
