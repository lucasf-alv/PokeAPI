...
class DockerSandboxSession extends unixLocal_1.UnixLocalSandboxSession {
    ...
    async resolveFilesystemRunAs(runAs) {
        if (runAs && runAs.trim().length > 0) {
            throw new errors_1.UserError('DockerSandboxClient does not support runAs for host-side materialization. Use container file APIs or execCommand instead.');
        }
        return undefined;
    }
    createEditor(runAs) {
    ...
    assertSessionUsable() {
        ...
        if (this.containerClosed || this.closeStarted) {
            throw new errors_1.UserError('Docker sandbox session is closed.');
        }
    }
    async viewImage(args) {
        const bytes = await this.readDockerFileAs(args.path, args.runAs, {
            maxBytes: media_1.MAX_VIEW_IMAGE_BYTES,
            image: true,
        });
    ...
    async listDir(args) {
        ...
        const output = await this.runReadableDockerFilesystemCommand([
            `test -d ${(0, shell_1.shellQuote)(absolutePath)} && find -H ${(0, shell_1.shellQuote)(absolutePath)} -mindepth 1 -maxdepth 1 -printf '%y\\0%f\\0'`,
    ...
    resolveSandboxPath(path, options = {}) {
        ...
        if (mountPath) {
            throw new errors_1.UserError(`DockerSandboxClient cannot map Docker volume mount path "${path ?? mountPath}" to a host path. Use the session file APIs or execCommand for container-visible paths under "${mountPath}".`);
        }
        const inContainerMountPath = dockerInContainerMountContainingPath(this.state.manifest, path);
        if (inContainerMountPath) {
            throw new errors_1.UserError(`DockerSandboxClient cannot map in-container mount path "${path ?? inContainerMountPath}" to a host path. Use the session file APIs or execCommand for container-visible paths under "${inContainerMountPath}".`);
        }
    ...
    resolveContainerFilesystemPath(path, options = {}) {
        ...
        if (selectedGrant &&
            !this.mountedPathGrants.some((grant) => grant.path === selectedGrant.path &&
                grant.readOnly === selectedGrant.readOnly &&
                (0, hostPath_1.sandboxPathGrantHostPath)(grant) ===
                    (0, hostPath_1.sandboxPathGrantHostPath)(selectedGrant))) {
            throw new errors_1.UserError(`Docker path grant "${selectedGrant.path}" is not mounted. Resume or recreate the session before using it.`);
        }
        if (options.forWrite) {
            const mount = this.state.manifest
                .mountTargets()
                .map(({ logicalPath, entry }) => ({
                path: resolveDockerMountPath(this.state.manifest.root, logicalPath, entry),
                entry,
            }))
                .sort((left, right) => right.path.length - left.path.length)
                .find((mount) => pathWithinDockerMount(resolved.path, mount.path));
            if (mount && (mount.entry.readOnly ?? true)) {
                throw new errors_1.UserError(`Docker path "${resolved.path}" is inside a read-only mount.`);
            }
        }
        return resolved.path;
    }
    async validateContainerFilesystemPath(path, options = {}) {
        const absolutePath = this.resolveContainerFilesystemPath(path, options);
        const output = await resolveTrustedDockerPath(this, absolutePath, 'resolve a filesystem path');
        const resolved = output.endsWith('\n') ? output.slice(0, -1) : output;
        if (!resolved.startsWith('/')) {
            throw new errors_1.UserError('Docker filesystem path resolution returned an invalid path.');
        }
        if (this.resolveContainerFilesystemPath(resolved, options) !== resolved) {
            throw new errors_1.UserError('Docker filesystem path normalization changed the resolved target.');
        }
    ...
    async readDockerFileAs(path, runAs, limits) {
        path = await this.validateContainerFilesystemPath(path);
        const imageLimit = limits?.image ? limits.maxBytes : undefined;
        const fileGuard = `if [ -e ${(0, shell_1.shellQuote)(path)} ] && [ ! -f ${(0, shell_1.shellQuote)(path)} ]; then exit 67; fi; `;
        const imageGuard = imageLimit === undefined
            ? ''
            : `size=$(wc -c < ${(0, shell_1.shellQuote)(path)}) || exit 1; if [ "$size" -gt ${imageLimit} ]; then exit 68; fi; `;
        const limit = limits?.maxBytes;
        // GNU base64 adds one newline per 76 encoded bytes, including the last line.
        const encodedBytes = limit === undefined ? undefined : Math.ceil(limit / 3) * 4;
        const output = await this.runReadableDockerFilesystemCommand(`${fileGuard}${imageGuard}base64 -- ${(0, shell_1.shellQuote)(path)}`, {
            runAs,
            maxOutputBytes: encodedBytes === undefined
                ? undefined
                : encodedBytes + Math.ceil(encodedBytes / 76),
        }, `read file ${path}`, path, imageLimit !== undefined);
        const bytes = Buffer.from(output.replace(/\s+/gu, ''), 'base64');
        if (limit !== undefined && bytes.byteLength > limit) {
            throw new errors_1.UserError(`Docker file exceeds the ${limit} byte read limit: ${path}`);
        }
        return bytes;
    }
    async runReadableDockerFilesystemCommand(command, options, action, path, image = false) {
        const result = await this.runDockerFilesystemCommand(command, options);
        if (result.status === 67)
            throw new errors_1.UserError(`${image ? 'Image' : 'File'} path is not a file: ${path}`);
        if (image) {
            if (result.status === 68)
        ...
        if (result.status !== 0) {
            if (result.status === 1 &&
                ...
                const exists = await (0, pathProbe_1.probeSandboxPathExists)({
                    ...
                    createError: () => new errors_1.UserError(`DockerSandboxClient failed to ${action}: ${(0, runProcess_1.formatSandboxProcessError)(result)}`),
                });
                if (!exists) {
                    if (image)
                        throw new errors_1.UserError(`Image file not found: ${path}`);
    ...
    async writeDockerTextFileAs(path, content, runAs, exclusive = false, moveSource) {
        ...
        const guard = exclusive
            ? `if [ -e ${(0, shell_1.shellQuote)(path)} ] || [ -L ${(0, shell_1.shellQuote)(path)} ]; then echo 'File already exists.' >&2; exit 1; fi; set -C; `
            : `if [ -e ${(0, shell_1.shellQuote)(path)} ] && [ ! -f ${(0, shell_1.shellQuote)(path)} ]; then echo 'Destination is not a regular file.' >&2; exit 1; fi; `;
        const moveGuard = moveSource
            ? `if [ ${(0, shell_1.shellQuote)(moveSource)} -ef ${(0, shell_1.shellQuote)(path)} ]; then echo 'Cannot move a file onto itself.' >&2; exit 1; fi; `
            : '';
        const write = `${moveGuard}${guard}cat > ${(0, shell_1.shellQuote)(path)}`;
        await this.runCheckedDockerFilesystemCommand(parent === '/' || parent === '.'
            ? write
            : `mkdir -p -- ${(0, shell_1.shellQuote)(parent)} && (${write})`, { runAs, input: content }, `write file ${path}`);
    }
    async deleteDockerPathAs(path, runAs) {
        path = await this.validateContainerFilesystemPath(path, { forWrite: true });
        await this.runCheckedDockerFilesystemCommand(`rm -- ${(0, shell_1.shellQuote)(path)}`, { runAs }, `delete path ${path}`);
    }
    ...
    async runDockerMountCommand(command, action, options = {}) {
        ...
        try {
            result = await this.runDockerFilesystemCommand(command, {
                runAs: 'root',
                input: options.input,
                mountEnvironment: options.environment ??
    ...
    async runDockerFilesystemCommand(command, options = {}) {
        ...
        if (options.mountEnvironment) {
            ...
            dockerArgs.push('/usr/bin/env', '-i', 'PATH=/usr/bin:/bin', 'LC_ALL=C', '/bin/sh', '-c', command);
        }
        return await runDockerProcess(dockerArgs, options.input, options.maxOutputBytes);
    }
    async close() {
        this.closeStarted = true;
        await this.closeContainerResources();
    }
    async closeContainerResources() {
        let cleanupError;
...
class DockerSandboxEditor {
    ...
    async createFile(operation) {
        ...
        await this.session.writeDockerTextFileAs(operation.path, content, this.runAs, true);
        return {};
    }
    async updateFile(operation) {
        const path = await this.session.validateContainerFilesystemPath(operation.path, {
            forWrite: true,
        });
        ...
        await this.session.writeDockerTextFileAs(destination, next, this.runAs, false, destination !== path ? path : undefined);
...
async function runDockerProcess(args, input, maxOutputBytes) {
    ...
    const captureOutput = (chunks, chunk) => {
        ...
        if (maxOutputBytes !== undefined && outputBytes > maxOutputBytes) {
            outputLimitError = new Error('Docker filesystem command exceeded its output limit.');
...
function dockerFileEnvironmentArgs(environment) {
