...
import { imageOutputFromBytes, MAX_VIEW_IMAGE_BYTES } from "../shared/media.mjs";
...
export class DockerSandboxSession extends UnixLocalSandboxSession {
    ...
    async resolveFilesystemRunAs(runAs) {
        if (runAs && runAs.trim().length > 0) {
            throw new UserError('DockerSandboxClient does not support runAs for host-side materialization. Use container file APIs or execCommand instead.');
        }
        return undefined;
    }
    createEditor(runAs) {
    ...
    resolveSandboxPath(path, options = {}) {
        ...
        if (mountPath) {
            throw new UserError(`DockerSandboxClient cannot map Docker volume mount path "${path ?? mountPath}" to a host path. Use the session file APIs or execCommand for container-visible paths under "${mountPath}".`);
        }
        const inContainerMountPath = dockerInContainerMountContainingPath(this.state.manifest, path);
        if (inContainerMountPath) {
            throw new UserError(`DockerSandboxClient cannot map in-container mount path "${path ?? inContainerMountPath}" to a host path. Use the session file APIs or execCommand for container-visible paths under "${inContainerMountPath}".`);
        }
    ...
    resolveContainerFilesystemPath(path, options = {}) {
        ...
        if (selectedGrant &&
            ...
            throw new UserError(`Docker path grant "${selectedGrant.path}" is not mounted. Resume or recreate the session before using it.`);
        }
        if (options.forWrite) {
            ...
            if (mount && (mount.entry.readOnly ?? true)) {
                throw new UserError(`Docker path "${resolved.path}" is inside a read-only mount.`);
            }
        }
    ...
    async validateContainerFilesystemPath(path, options = {}) {
        ...
        if (!resolved.startsWith('/')) {
            throw new UserError('Docker filesystem path resolution returned an invalid path.');
        }
        if (this.resolveContainerFilesystemPath(resolved, options) !== resolved) {
            throw new UserError('Docker filesystem path normalization changed the resolved target.');
        }
    ...
    async readDockerFileAs(path, runAs, limits) {
        ...
        const output = await this.runReadableDockerFilesystemCommand(`${fileGuard}${imageGuard}base64 -- ${shellQuote(path)}`, {
            runAs,
        ...
        if (limit !== undefined && bytes.byteLength > limit) {
            throw new UserError(`Docker file exceeds the ${limit} byte read limit: ${path}`);
        }
        return bytes;
    }
    async runReadableDockerFilesystemCommand(command, options, action, path, image = false) {
        ...
        if (result.status === 67)
            throw new UserError(`${image ? 'Image' : 'File'} path is not a file: ${path}`);
        if (image) {
            if (result.status === 68)
        ...
        if (result.status !== 0) {
            if (result.status === 1 &&
                ...
                const exists = await probeSandboxPathExists({
                    ...
                    createError: () => new UserError(`DockerSandboxClient failed to ${action}: ${formatSandboxProcessError(result)}`),
                });
                if (!exists) {
                    if (image)
    ...
    async writeDockerTextFileAs(path, content, runAs, exclusive = false, moveSource) {
        ...
        await this.runCheckedDockerFilesystemCommand(parent === '/' || parent === '.'
            ? write
            : `mkdir -p -- ${shellQuote(parent)} && (${write})`, { runAs, input: content }, `write file ${path}`);
    }
    async deleteDockerPathAs(path, runAs) {
        path = await this.validateContainerFilesystemPath(path, { forWrite: true });
        await this.runCheckedDockerFilesystemCommand(`rm -- ${shellQuote(path)}`, { runAs }, `delete path ${path}`);
    }
