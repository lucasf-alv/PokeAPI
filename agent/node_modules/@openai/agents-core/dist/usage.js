...
function cloneRequestUsage(input) {
...
class Usage {
    ...
    add(newUsage) {
        ...
        if (newUsage.inputTokensDetails) {
            ...
            this.inputTokensDetails.push(...newUsage.inputTokensDetails.map(cloneTokenDetails));
        }
        if (newUsage.outputTokensDetails) {
            // The type does not allow undefined, but it could happen runtime
            this.outputTokensDetails.push(...newUsage.outputTokensDetails.map(cloneTokenDetails));
        }
        if (Array.isArray(newUsage.requestUsageEntries) &&
            newUsage.requestUsageEntries.length > 0) {
            this.requestUsageEntries ??= [];
            this.requestUsageEntries.push(...newUsage.requestUsageEntries.map(cloneRequestUsage));
        }
        else if (newUsage.requests === 1 && newUsage.totalTokens > 0) {
            this.requestUsageEntries ??= [];
            this.requestUsageEntries.push(cloneRequestUsage({
