...
exports.AgentAsToolInputSchema = void 0;
exports.defaultInputBuilder = defaultInputBuilder;
exports.resolveAgentToolInput = resolveAgentToolInput;
exports.buildStructuredInputSchemaInfo = buildStructuredInputSchemaInfo;
const zod_1 = require("zod");
const zodCompat_1 = require("./utils/zodCompat.js");
const tools_1 = require("./utils/tools.js");
const zodJsonSchemaCompat_1 = require("./utils/zodJsonSchemaCompat.js");
...
exports.AgentAsToolInputSchema = zod_1.z.object({
    input: zod_1.z.string(),
});
function defaultInputBuilder(options) {
...
async function resolveAgentToolInput(options) {
    ...
    if ((0, typeGuards_1.isAgentToolInput)(options.params) && hasOnlyInputField(options.params)) {
...
function buildStructuredInputSchemaInfo(params, toolName, includeJsonSchema) {
    if (!params) {
        return {};
    }
    const summary = buildSchemaSummary(params);
    const jsonSchema = includeJsonSchema
        ? (0, tools_1.getSchemaAndParserFromInputType)(params, toolName, { strict: true }).schema
        : undefined;
...
function buildSchemaSummary(parameters) {
    if ((0, typeGuards_1.isZodObject)(parameters)) {
        const summary = summarizeZodSchema(parameters);
        return summary ? formatSchemaSummary(summary) : undefined;
    }
    if ((0, standardSchema_1.isStandardSchemaWithJSON)(parameters)) {
        const summary = summarizeJsonSchema((0, standardSchema_1.standardSchemaToJsonSchema)(parameters, 'input'));
        return summary ? formatSchemaSummary(summary) : undefined;
    }
    if ((0, zodJsonSchemaCompat_1.hasJsonSchemaObjectShape)(parameters)) {
...
function summarizeZodSchema(schema) {
    ...
    const description = (0, zodCompat_1.readZodDescription)(schema);
    if (description) {
        hasDescription = true;
    }
    if (!hasDescription) {
