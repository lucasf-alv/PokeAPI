 ...
 */
function applyDiff(input, diff, mode = 'default') {
...
function updateLineEnding(input) {
...
function advanceCursorToAnchor(anchor, inputLines, cursor, parser, requireMatch = false, forceForwardSearch = false) {
    let found = false;
    const hasExactMatchBeforeCursor = !forceForwardSearch &&
    ...
    if (!found) {
        const hasTrimmedMatchBeforeCursor = !forceForwardSearch &&
...
function findContext(lines, context, start, eof) {
    if (eof) {
        const searchLines = lines.length > 1 && lines[lines.length - 1] === ''
            ? lines.slice(0, -1)
            : lines;
...
function applyChunks(input, chunks, lineEnding) {
