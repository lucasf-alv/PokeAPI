...
import { getDefaultModelSettings, isGpt5OrNewerReasoningModel, isGpt5OrNewerDefault, } from "./defaultModel.mjs";
...
import { assertStandardSchemaObjectRoot, hasStandardSchemaMarker, isStandardSchemaWithJSON, standardSchemaToJsonSchema, validateStandardSchema, unsupportedStandardSchemaError, } from "./utils/standardSchema.mjs";
import { prepareOpenAIStrictToolSchema } from "./utils/strictToolSchema.mjs";
...
export class Agent extends AgentHooks {
    ...
    constructor(config) {
        ...
        if (config.outputType) {
            if (!isZodObject(config.outputType) &&
            ...
            if (!isZodObject(config.outputType) &&
                isStandardSchemaWithJSON(config.outputType)) {
                const inputSchema = standardSchemaToJsonSchema(config.outputType, 'input');
                assertStandardSchemaObjectRoot(inputSchema, 'Agent output');
                prepareOpenAIStrictToolSchema(inputSchema);
            }
    ...
    processFinalOutput(output) {
        ...
        if (typeof this.outputType === 'object' ||
            ...
            if (isStandardSchemaWithJSON(this.outputType)) {
                const inputSchema = standardSchemaToJsonSchema(this.outputType, 'input');
                assertStandardSchemaObjectRoot(inputSchema, 'Agent output');
                const normalized = prepareOpenAIStrictToolSchema(inputSchema).normalizeInput(parsed);
                return validateStandardSchema(this.outputType, normalized);
            }
