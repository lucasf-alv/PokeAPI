...
function isNoopSpan(span) {
    return (span.traceId === utils_1.NOOP_TRACE_OR_SPAN_ID ||
        span.spanId === utils_1.NOOP_TRACE_OR_SPAN_ID);
}
/**
...
class ConsoleSpanExporter {
    async export(items) {
        ...
        for (const item of items) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                console.log(item.type === 'trace'
...
class BatchTraceProcessor {
    ...
    #activeExportAbortControllers = new Set();
    #activeExportPromises = new Set();
    ...
    async #exportBatches(force = false, signal) {
        ...
        const exportBatch = async (batch) => {
            ...
            const combinedSignal = (0, abortSignals_1.combineAbortSignals)(signal, activeExportAbortController.signal);
            const exportPromise = (async () => {
                try {
                    await this.#exporter.export(batch, combinedSignal.signal);
                }
                catch (error) {
                    (0, logger_1.logModelAndToolActionError)(logger_1.default, 'Tracing exporter failed to export batch', error);
                }
                finally {
...
class MultiTracingProcessor {
    ...
    async dispatchTrace(trace) {
        if (trace.traceId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
            return;
        }
